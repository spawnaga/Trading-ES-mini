...
def renko_df(df_raw, ATR):
    """Convert time-price chart to a Renko chart. For more information check
    https://www.investopedia.com/terms/r/renkochart.asp#:~:text=A%20Renko%20chart%20is%20a,like%20a%20series%20of%20bricks."""
    df_raw = df_raw[-500:]
...
class get_data:
    ...
    def next_weekday(d, weekday):
        ...
        if days_ahead <= 0:  # Target day already happened this week
            days_ahead += 7
        date_to_return = d + timedelta(days_ahead) + timedelta(
            days=7)  # 0 = Monday, 1=Tus self.ES day, 2=Wed self.ES day...
    ...
    def ES(self, ES, ATR):
        ...
        ES_df['roll_max_cp'] = ES_df['high'].rolling(int(20 / ES_df['ATR'].iloc[-1])).max()
        ES_df['roll_min_cp'] = ES_df['low'].rolling(int(20 // ES_df['ATR'].iloc[-1])).min()
...
class Trade():
    ...
    def trade(self, ES, hasNewBar=None):
        """ Get tick data simultaneously for ES mini movements and send to strategy function"""
        if self.submitted == 1:
        ...
        if self.stock_owned[0] > 0:
            ...
            print(f'current proft/loss = {(self.call_option_price.bid - self.call_cost) * 50}')

        elif self.stock_owned[1] > 0:
            print(f'Put cost was = {self.put_cost}')
            print(f'current profit/loss = {(self.put_option_price.bid - self.put_cost) * 50}')
    ...
    def strategy(self, df):
        ...
        print(
            ...
            f'{self.put_option_price.bid} and'
            f'RSI = {df["RSI"].iloc[i - 2]} and '
        ...
        elif (self.stock_owned[0] > 0) and (self.call_option_price.bid - self.call_cost >= 1.02) and (
        ...
        elif (self.stock_owned[1] > 0) and (self.put_option_price.bid - self.put_cost >= 1.01) and (
        ...
        elif self.barnumb_lock is True:
            # self.submitted = 0
            self.barnumb_lock = False
            self.barnumb_value = 0
            return buy_index, sell_index, take_profit
    ...
    def error(self, reqId=None, errorCode=None, errorString=None, contract=None):  # error handler
        ...
        if errorCode in [2104, 2108, 2158, 10182, 1102, 2106, 2107] and len(self.reqId) < 1:
            ...
            try:
                ...
                self.error(reqId,errorCode,errorString)
            # ES-Mini futures contract
            self.ES = ib.reqHistoricalData(contract=ES, endDateTime='', durationStr='2 D',
                                           barSizeSetting='1 min', whatToShow='TRADES', useRTH=False, keepUpToDate=True,
                                           timeout=10)  # start data collection for ES-Mini
    ...
    def take_profit(self, contract, price):  # start taking profit
        """ Taking profits from wining options"""
    ...
    def open_position(self, contract, quantity, price):  # start position
        """ Starting a new position for options"""
    ...
    def account_update(self, value=None):
        """ Update current account value and PNLs """
        self.update += 1
